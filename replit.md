...
- July 9, 2025. Implemented comprehensive Ctrl+V paste functionality across all CTQ analysis components - added Ctrl+V keyboard shortcut to ProcessCapability component for Excel data paste (previously only had Ctrl+Z), fixed MSA component by removing input field detection restrictions, ensured all three components (ProcessCapability, MSA, ContCTQHypTesting) now have consistent keyboard shortcuts that work when the specific CTQ tab is active regardless of focus state, cleaned up console logs from ProcessCapability component
