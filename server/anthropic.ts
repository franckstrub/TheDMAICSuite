...
if (
  ...
  console.warn(
    "Warning: ANTHROPIC_API_KEY is missing or has incorrect format. API calls will fail.",
  );
}
...
export async function generateElevatorSpeech(
  ...
  try {
    ...
    if (!process.env.ANTHROPIC_API_KEY) {
      ...
      throw new Error(
        "ANTHROPIC_API_KEY is missing. Please make sure it is set in your environment variables.",
      );
    }
    ...
    const problemStatement =
      charterData?.problemStatement || "No problem statement provided";
...
${formattedCustomerRequirements
  .map(
    (req: any) =>
      `- ${req.need} (Importance: ${req.importance}/5, CTS: ${req.ctq})`,
  )
  .join("\n")}

Business Requirements (Voice of Business):
${formattedBusinessRequirements
  .map(
    (req: any) =>
      `- ${req.requirement}: ${req.need} (Importance: ${req.importance}/5, CTS: ${req.ctq})`,
  )
  .join("\n")}
...
Keep it concise, compelling, and free of technical jargon.`;
    ...
    if (response.content[0].type === "text") {
      ...
      return (
        "AI has generated this elevator speech based on your project data:\n\n" +
    ...
    if (error.status === 401) {
      throw new Error(
        "Authentication failed: Invalid API key. Please check your ANTHROPIC_API_KEY environment variable.",
      );
    } else if (error.status === 400) {
      throw new Error(
        `Bad request: ${error.message || "Check if the model name is correct and the request format is valid."}`,
      );
    } else if (error.status === 404) {
      throw new Error(
        "Resource not found: The specified model may not exist or be available.",
      );
      ...
      throw new Error(
        `Failed to generate elevator speech: ${error.message || "Unknown error"}`,
      );
    }
  }
}
...
export async function generateMitigationPlan(
  ...
  try {
    console.log(
      `Generating mitigation plan for risk: "${riskName}" with probability: "${probability}" and impact: "${impact}"`,
    );
...
The mitigation plan should be specific to this risk, considering its probability and impact level.`;
    ...
    if (error.status === 401) {
      ...
      console.error("Full error details:", JSON.stringify(error, null, 2));
      throw new Error(
        `Failed to generate mitigation plan: ${error.message || "Unknown error"}`,
      );
    }
  }
}
