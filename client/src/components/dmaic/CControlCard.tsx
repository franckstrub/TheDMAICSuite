...
export function CControlCard({ projectId, ctqName }: CControlCardProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: { 
      ...
      return apiRequest(
        'POST',
        `/api/projects/${projectId}/spc/c/${encodeURIComponent(ctqName)}`,
        payload
      );
    },
    onSuccess: () => {
      toast({
        title: "Data saved",
        description: "C control card data saved successfully.",
      });
  ...
  const saveAiAnalysisMutation = useMutation({
    mutationFn: async (analysis: string) => {
      return apiRequest(
        'PATCH',
        `/api/projects/${projectId}/spc/c/${encodeURIComponent(ctqName)}/ai-analysis`,
        { aiAnalysis: analysis }
      );
    },
    onSuccess: () => {
      toast({
        ...
        description: "C chart AI analysis saved successfully.",
      });
      queryClient.invalidateQueries({
        queryKey: [`/api/projects/${projectId}/spc/c/${encodeURIComponent(ctqName)}`]
      });
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent) => {
    ...
    toast({
      title: "Data pasted",
      description: `Successfully pasted ${newDataValues.length} defect counts`,
    });
  ...
  const handleSaveData = useCallback(() => {
    ...
    if (validValues.length < 2) {
      toast({
        ...
        description: "Need at least 2 data points for C chart",
        variant: "destructive",
      });
      return;
    }
  ...
  const generateAIAnalysis = useCallback(async () => {
    ...
    try {
      const cBar = validData.reduce((a, b) => a + b, 0) / validData.length;
      ...
      const stageStatsForAI = stagesEnabled && localValidIndices.length > 0 ? (() => {
        ...
        return Array.from(stageMap.entries()).map(([stageName, values]) => {
          const stageCBar = values.reduce((a, b) => a + b, 0) / values.length;
          return {
            stageName,
            count: values.length,
            mean: stageCBar,
            ucl: stageCBar + 3 * Math.sqrt(stageCBar),
            lcl: Math.max(0, stageCBar - 3 * Math.sqrt(stageCBar)),
          };
        });
      })() : undefined;
      ...
      const response = await fetch(`/api/projects/${projectId}/spc/c/${encodeURIComponent(ctqName)}/ai-analysis`, {
        method: 'POST',
      ...
      if (data?.assessment) {
        ...
        toast({
          ...
          description: "C chart analysis has been generated successfully",
        });
      } else {
  ...
  const calculateCStats = useCallback(() => {
    if (!hasValidData) return null;

    const n = validDataValues.length;
    const cBar = validDataValues.reduce((a, b) => a + b, 0) / n;
  ...
  const calculateStageStats = useCallback((): StageStats[] => {
    ...
    const processStage = (stageName: string, startIdx: number, endIdx: number) => {
      ...
      const cBar = stageDataValues.reduce((a, b) => a + b, 0) / stageDataValues.length;
  ...
  const cChartPoints = getCChartPointArrays();

  const generateCChartStageTraces = () => {
    ...
    stageStatsList.forEach((stageStat, idx) => {
      ...
      traces.push({
        ...
        type: 'scatter',
        mode: 'lines',
        name: idx === 0 ? 'Centerline (c̄)' : undefined,
        showlegend: idx === 0,
  ...
  const cChartTraces: any[] = [];
  
  if (stats) {
    if (stagesEnabled && stageStatsList.length > 0) {
      cChartTraces.push(...generateCChartStageTraces());
    } else {
      cChartTraces.push({
        x: chartXIndices,
    ...
    if (cChartPoints.outOfControl.x.length > 0) {
      cChartTraces.push({
        ...
        y: cChartPoints.outOfControl.y,
        type: 'scatter',
        mode: 'markers',
        name: 'Out of Control',
        marker: { color: '#dc2626', size: 10, symbol: 'circle' },
        hovertemplate: 'Sample %{x}<br>Count: %{y} (OOC)<extra></extra>',
      });
    }
  }
  ...
  const cChartLayout = {
    title: {
      text: `C (Count) Chart of ${indicatorName}`,
      font: { size: 16 },
    },
    xaxis: getXAxisConfig(),
    yaxis: {
      title: { text: 'Defect Count (c)' },
      showgrid: true,
      gridcolor: '#e5e7eb',
      zeroline: true,
                ...
                <Plot
                  ...
                  config={{
                    ...
                    toImageButtonOptions: {
                      format: 'png',
                      filename: `C_Control_Card_of_${ctqName}`,
                      height: 500,
                      width: 800,
                      scale: 1
                    }
