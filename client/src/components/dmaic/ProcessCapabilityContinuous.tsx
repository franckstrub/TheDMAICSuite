...
interface ProcessCapabilityContinuousCardsProps {
  ...
  calculateProcessCapabilityStats: (ctqId: string) => ProcessCapabilityStats | null;
  // Make sure formatPercentage is accepted as a prop if passed
  formatPercentage: (value: number | undefined, dpmoValue: number | undefined) => string;
}
...
export function ProcessCapabilityContinuousCards({
  ...
  const renderCapabilityDetails = () => {
    ...
    if (capabilityIndex === "Cp/Cpk") {
      return (
        <>
          <div className="flex justify-between" title="Pp: = (USL − LSL) / 6σLT">
            <span>Pp:</span>
            <span className="font-medium">{stats.pp !== null ? stats.pp.toFixed(3) : "N/A"}</span>
          </div>
          <div className="flex justify-between" title="Ppk = min[(USL - μ) / 3σLT, (μ - LSL) / 3σLT]">
            <span>Ppk:</span>
            <span className="font-medium">{stats.ppk !== null ? stats.ppk.toFixed(3) : "N/A"}</span>
          </div>
          <div className="flex justify-between" title="Cp: = (USL − LSL) / 6σST">
            <span>Cp:</span>
            <span className="font-medium">{stats.cp !== null ? stats.cp.toFixed(3) : "N/A"}</span>
          </div>
          <div className="flex justify-between" title="Cpk = min[(USL - μ) / 3σST, (μ - LSL) / 3σST]">
            <span>Cpk:</span>
            <span className="font-medium">{stats.cpk !== null ? stats.cpk.toFixed(3) : "N/A"}</span>
          </div>
        </>
      );
    } else { // Z or Z-Equivalent
      return (
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span>Z-shift:</span>
            <span className="font-medium text-sm">{stats.zShift.toFixed(2)}σ</span>
          </div>
          ...
          {stats.isNormal ? ( // Normal Data (Z values)
            <>
              ...
              <div className="space-y-2 text-sm pl-2 border-l-2 border-green-200">
                ...
                {capabilityData[ctq]?.dataSetTerm === "Long Term" && (
                  <>
                    <div className="flex justify-between text-xs ml-2" title="Z_LSL LT = (μ - LSL) / σLT">
                      <span>• Z_LSL LT:</span>
                      <span className="font-medium text-xs">
                        {!isNaN(stats.zLSL_LT) ? stats.zLSL_LT.toFixed(2) + 'σ' : 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs ml-2" title="Z_USL LT = (USL - μ) / σLT">
                      <span>• Z_USL LT:</span>
                      <span className="font-medium text-xs">
                        {!isNaN(stats.zUSL_LT) ? stats.zUSL_LT.toFixed(2) + 'σ' : 'N/A'}
                      </span>
                    </div>
                  </>
                )}
              ...
              <div className="space-y-2 text-sm pl-2 border-l-2 border-blue-200">
                ...
                {capabilityData[ctq]?.dataSetTerm === "Short Term" && (
                  <>
                    <div className="flex justify-between text-xs ml-2" title="Z_LSL ST = (μ - LSL) / σST">
                      <span>• Z_LSL ST:</span>
                      <span className="font-medium text-xs">
                        {!isNaN(stats.zLSL_ST) ? stats.zLSL_ST.toFixed(2) + 'σ' : 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs ml-2" title="Z_USL ST = (USL - μ) / σST">
                      <span>• Z_USL ST:</span>
                      <span className="font-medium text-xs">
                        {!isNaN(stats.zUSL_ST) ? stats.zUSL_ST.toFixed(2) + 'σ' : 'N/A'}
                      </span>
                    </div>
                  </>
                )}
            ...
            <>
              ...
              <div className="space-y-2 text-sm pl-2 border-l-2 border-green-200">
                ...
                {capabilityData[ctq]?.dataSetTerm === "Long Term" && (
                  <>
                    <div className="flex justify-between text-xs ml-2" title="Z-Equivalent_LSL LT = (μ - LSL) / σLT">
                      <span>• Z-Equivalent_LSL LT:</span>
                      <span className="font-medium text-xs">
                        {stats.ZequivLSL_LT ? stats.ZequivLSL_LT.toFixed(2) + 'σ' : 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs ml-2" title="Z-Equivalent_USL LT = (USL - μ) / σLT">
                      <span>• Z_Equivalent_USL LT:</span>
                      <span className="font-medium text-xs">
                        {stats.ZequivUSL_LT ? stats.ZequivUSL_LT.toFixed(2) + 'σ' : 'N/A'}
                      </span>
                    </div>
                  </>
                )}
              ...
              <div className="space-y-2 text-sm pl-2 border-l-2 border-blue-200">
                ...
                {capabilityData[ctq]?.dataSetTerm === "Short Term" && (
                  <>
                    <div className="flex justify-between text-xs ml-2" title="Z-Equivalent_LSL ST = (μ - LSL) / σST">
                      <span>• Z-Equivalent_LSL ST:</span>
                      <span className="font-medium text-xs">
                        {stats.ZequivLSL_ST ? stats.ZequivLSL_ST.toFixed(2) + 'σ' : 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs ml-2" title="Z-Equivalent_USL ST = (USL - μ) / σST">
                      <span>• Z_Equivalent_USL ST:</span>
                      <span className="font-medium text-xs">
                        {stats.ZequivUSL_ST ? stats.ZequivUSL_ST.toFixed(2) + 'σ' : 'N/A'}
                      </span>
                    </div>
                  </>
                )}
  ...
  return (
    <>
      ...
      {showStatistics[ctq] && stats && dataPoints[ctq] && dataPoints[ctq].length >= 25 && (
        <div className="mt-6">
          ...
          <div className={`grid grid-cols-1 gap-6 ${showPercentage ? "md:grid-cols-4" : "md:grid-cols-3"}`}>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between" title="SQRT( Σ(Xi-μ) / (n-1) )">
                  <span>Std Dev (σ):</span>
                ...
                {stats.pValue && (
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between text-xs" title={"Anderson-Darling test value"}>
                      <span> &nbsp;• AD-Value:</span>
                      <span className="font-medium text-xs">{stats.adStatistic.toFixed(5)}</span>
                    </div>
                    <div className="flex justify-between text-xs" title={"Anderson-Darling test p-value"}>
                      <span> &nbsp;• P-Value:</span>
                      <span className="font-medium text-xs">{stats.pValue.toFixed(5)}</span>
                    </div>
                  </div>
                )}
                <div>
                  ...
                  >
                    {stats.isNormal ? "Data follows normal distribution" : "Data does not follow normal distribution"}
                  </Badge>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  ...
                  {(capabilityData[ctq]?.capabilityIndex === "Z" ||
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm pl-2 border-l-2 border-green-200">
                        {/* Yield */}
                        <div className="flex justify-between" title="Yield LT = 100% - % defects LT">
                          ...
                          {stats.isNormal && capabilityIndex === "Z" ? (
                            ...
                            <span className="font-medium">
                              {formatPercentage(stats.obsYieldLT, stats.obsDPMOLT)}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex justify-between">
                          ...
                          {stats.isNormal && capabilityIndex === "Z" ? (
                            ...
                            <span className="font-medium" title="Nbr of total defects LT / Nbr of data LT">
                              {formatPercentage(stats.obspercentDefectsLT, stats.obsDPMOLT)}
                            </span>
                          ) : (
                            <span
                              className="font-medium"
                        ...
                        {capabilityData[ctq]?.dataSetTerm === "Long Term" && (
                          <div className="flex justify-between text-xs ml-2">
                            ...
                            {stats.isNormal && capabilityIndex === "Z" ? (
                              ...
                              <span className="font-medium" title="Nbr of defects_LSL LT / Nbr of data LT">
                                {formatPercentage(stats.obspdLSL_LT, stats.obsDPMOLT)}
                              </span>
                            ) : (
                              <span
                                className="font-medium"
                        ...
                        {capabilityData[ctq]?.dataSetTerm === "Long Term" && (
                          <div className="flex justify-between text-xs ml-2">
                            ...
                            {stats.isNormal && capabilityIndex === "Z" ? (
                              ...
                              <span className="font-medium" title="Nbr of defects_USL LT / Nbr of data LT">
                                {formatPercentage(stats.obspdUSL_LT, stats.obsDPMOLT)}
                              </span>
                            ) : (
                              <span
                                className="font-medium"
                  ...
                  {(capabilityData[ctq]?.capabilityIndex === "Z" ||
                    ...
                    <div>
                      <h5 className="font-medium text-blue-700 mb-2 text-sm">Short Term</h5>
                      <div className="space-y-2 text-sm pl-2 border-l-2 border-blue-200">
                        <div className="flex justify-between" title="Yield ST = 100% - % defects ST">
                          ...
                          {stats.isNormal && capabilityIndex === "Z" ? (
                            ...
                            <span className="font-medium">
                              {formatPercentage(stats.obsYieldST!, stats.obsDPMOST!)}
                            </span>
                          )}
                        </div>
                        <div className="flex justify-between">
                          ...
                          {stats.isNormal && capabilityIndex === "Z" ? (
                            ...
                            <span className="font-medium" title="Nbr of total defects ST / Nbr of data ST">
                              {formatPercentage(stats.obspercentDefectsST, stats.obsDPMOST)}
                            </span>
                          ) : (
                            <span
                              className="font-medium"
                        ...
                        {capabilityData[ctq]?.dataSetTerm === "Short Term" && (
                          <div className="flex justify-between text-xs ml-2">
                            ...
                            {stats.isNormal && capabilityIndex === "Z" ? (
                              ...
                              <span className="font-medium" title="Nbr of defects_LSL ST / Nbr of data ST">
                                {formatPercentage(stats.obspdLSL_ST, stats.obsDPMOLT)}
                              </span>
                            ) : (
                              <span
                                className="font-medium"
                        ...
                        {capabilityData[ctq]?.dataSetTerm === "Short Term" && (
                          <div className="flex justify-between text-xs ml-2">
                            ...
                            {stats.isNormal && capabilityIndex === "Z" ? (
                              ...
                              <span className="font-medium" title="Nbr of defects_USL ST / Nbr of data UT">
                                {formatPercentage(stats.obspdUSL_ST, stats.obsDPMOLT)}
                              </span>
                            ) : (
                              <span
                                className="font-medium"
