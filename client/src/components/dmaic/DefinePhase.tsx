...
import exportPdfLock from "@/lib/exportPdfLock";
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      try {
        ...
        const pdfCanvas = await html2canvas(charterElement, {
          ...
          onclone: (clonedDoc) => {
            ...
            if (clonedCharter) {
              ...
              // Update all form elements using data-field attributes
        ...
        exportPdfLock.markAsGenerated();
        console.log("PDF generation complete, marked as generated with timestamp:", Date.now());
        ...
        // IMPORTANT: If we already generated a PDF successfully, don't try fallback method at all
        if (exportPdfLock.wasGenerated()) {
          console.log("PDF was already generated successfully, skipping fallback method entirely");
        ...
        console.log("No PDF generated yet. Attempting alternative PDF generation method...");
        ...
        try {
          ...
          if (totalPages <= 1) {
            ...
            pdf.addImage(pageImgData, 'JPEG', 10, 40, pdfImgWidth, pdfImgHeight);
          } else {
            // Slice image across pages
            const pageHeight = pdfHeight - 20 - 15; // margins and footer
            for (let page = 0; page < totalPages; page++) {
              if (page > 0) pdf.addPage();
              
              const posY = page === 0 ? 40 : 10;
              const srcY = (page * pageHeight / pdfImgHeight) * fallbackCanvas.height;
              const srcHeight = Math.min(fallbackCanvas.height - srcY, (pageHeight / pdfImgHeight) * fallbackCanvas.height);
              const printHeight = (srcHeight / fallbackCanvas.height) * pdfImgHeight;
              
              const tempCanvas = document.createElement('canvas');
              tempCanvas.width = fallbackCanvas.width;
              ...
              if (tempCtx) {
                tempCtx.drawImage(
                  fallbackCanvas, 
                  0, srcY, fallbackCanvas.width, srcHeight,
                  0, 0, tempCanvas.width, tempCanvas.height
                );
          ...
          // Check one more time if a PDF was generated by the primary process
          if (exportPdfLock.wasGenerated()) {
            console.log("PDF was already generated by primary method, skipping fallback save");
            return;
          }
          
          // Save the PDF with fallback method
          pdfOutput = `${safeFilename}_Project_Charter_${format(new Date(), 'yyyy-MM-dd')}.pdf`;
          pdf.save(pdfOutput);
          
          // Mark as generated BEFORE showing toast to avoid race conditions
          exportPdfLock.markAsGenerated();
          console.log("PDF generation by fallback method complete, marked as generated with timestamp:", Date.now());
          ...
          // Check again if a PDF was generated by the primary method in the meantime
          if (exportPdfLock.wasGenerated()) {
            console.log("PDF was already generated, skipping error message");
            return;
          }
          
          // Only show error if no PDF was generated by either method
          toast({
            title: "Export Failed",
