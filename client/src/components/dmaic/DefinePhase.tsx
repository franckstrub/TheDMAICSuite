...
export default function DefinePhase() {
  ...
  const [newBenefitCategory, setNewBenefitCategory] =
    useState<SoftBenefit["category"]>("employee");

  // State for collapsible sections - default to collapsed
  const [isFinancialSectionExpanded, setIsFinancialSectionExpanded] =
    useState(false);
  ...
  useEffect(() => {
    if (projectId) {
      const savedState = localStorage.getItem(
        `financial_section_expanded_${projectId}`,
      );
      if (savedState !== null) {
  ...
  useEffect(() => {
    if (projectId) {
      localStorage.setItem(
        `financial_section_expanded_${projectId}`,
        isFinancialSectionExpanded.toString(),
      );
    }
    ...
    if (sipoc?.sipoc && !sipocFormInitialized.current) {
      ...
      try {
        ...
        if (!currentProcessName && sipoc.sipoc.processName) {
          console.log(
            "Process name not set correctly, setting directly:",
            sipoc.sipoc.processName,
          );
  ...
  const [requirements, setRequirements] = useState([
    {
      requirement: "Fast delivery",
      customerRequirement: "Delivery within 24 hours",
      importance: 4,
      satisfaction: 2,
    },
    {
      requirement: "Order accuracy",
      customerRequirement: "100% accurate order fulfillment",
      importance: 5,
      satisfaction: 3,
    },
    {
  ...
  useEffect(() => {
    ...
    if (charter?.charter) {
        ...
        try {
          ...
          if (typeof charter.charter.softBenefits === "string") {
            ...
            if (benefitsStr.startsWith('"[') && benefitsStr.endsWith(']"')) {
              ...
              console.log(
                "Parsed soft benefits from double-quoted JSON string:",
                parsedBenefits,
              );
            }
            // Standard case: JSON array stored as a string
              ...
              console.log(
                "Parsed soft benefits from JSON string:",
                parsedBenefits,
              );
            }
            // Legacy format case
          ...
          console.error(
            "Error parsing soft benefits:",
            e,
            "Value was:",
            charter.charter.softBenefits,
          );
          setSoftBenefits([]);
        }
      }
      ...
      try {
        ...
        if (
          charter.charter.fteWorkingDaysPerYear &&
          ...
          const calculatedValue = parseFloat(
            charter.charter.fteCalculatedValue || "0",
          );
        ...
        // Fallback: if we don't have the parameters but have the FTE string
          ...
          const fteMatch =
            charter.charter.fteBenefits.match(/(\d+\.\d+)\s+FTE/);
          if (fteMatch && fteMatch[1]) {
            ...
            console.log(
              "Fallback: Parsed FTE value from string:",
              fteValue,
              "calculated value:",
              fteBenefitsValue,
            );
      ...
      charterForm.reset({
        ...
        projectCategory:
          charter.charter.projectCategory || "Process Improvement",
        ...
        workingCapitalGains:
          charter.charter.workingCapitalGains?.toString() || "",
  ...
  useEffect(() => {
    ...
    if (charterError || !charter || charter.isLoading) {
      ...
      if (currentProject?.targetEndDate && !currentControlDate) {
        const targetEndDate = new Date(currentProject.targetEndDate)
          .toISOString()
          .split("T")[0];
        console.log(
          "Initial load: Setting Control Phase date to match target end date:",
          targetEndDate,
        );
        ...
        console.log(
          "Not setting initial control phase date:",
          currentProject?.targetEndDate ? "Has target date" : "No target date",
          currentControlDate
            ? "Control date already set"
            : "No control date set",
        );
      }
    }
  ...
  useEffect(() => {
    ...
    if ((charterError || (charter && !charter.charter)) && currentProject) {
      console.log(
        "No charter found for project, initializing form with default values",
      );
  ...
  const saveCharterMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      console.log(
        "MUTATION DEBUG - Charter form - Project Title value:",
        data.projectTitle,
      );
      console.log(
        `MUTATION DEBUG - Project ID: ${projectId}, currentCharterId: ${charter?.charter?.id}`,
      );
      ...
      const payload = {
        ...
        stakeholderFunction:
          stakeholders.length > 0 ? stakeholders[0].function : "",
      ...
      if (charter && charter.charter && charter.charter.id) {
        ...
        return apiRequest(
          "PUT",
          `/api/charters/${charter.charter.id}`,
          payload,
        );
      } else {
    ...
    try {
      ...
      if (stakeholderForm) {
        ...
        const functionInput = document.querySelector(
          ".stakeholder-function-input",
        ) as HTMLInputElement;

        if (
          nameInput &&
          (nameInput.value.trim() ||
            (functionInput && functionInput.value.trim()))
        ) {
          console.log(
            "Found pending stakeholder data:",
            nameInput.value,
            functionInput?.value,
          );
      ...
      if (teamMemberForm) {
        ...
        if (
          ...
          console.log(
            "Found pending team member data:",
            nameInput.value,
            functionInput?.value,
          );
      ...
      data.totalFinancialSavings = charterForm.getValues(
        "totalFinancialSavings",
      );
      ...
      const preparedData = {
        ...
        // Keep legacy fields for backward compatibility
        stakeholder:
          workingStakeholders.length > 0 ? workingStakeholders[0].name : "",
        stakeholderFunction:
          workingStakeholders.length > 0 ? workingStakeholders[0].function : "",
      ...
      // Debug log
      console.log(
        "Submitting project charter with prepared data:",
        preparedData,
      );
  ...
  const updateTotalFinancialSavings = (overrideFteValue?: number) => {
    ...
    const qualityCostSavings =
      parseFloat(charterForm.getValues("savingsPerYear").toString()) || 0;
    const financialSavings =
      parseFloat(charterForm.getValues("financialSavings").toString()) || 0;

    // Get FTE benefits value - preferring the override value if provided
    const fteBenefits =
      overrideFteValue !== undefined
        ? overrideFteValue
        : Number(fteParams.calculatedValue) || 0;

    // Calculate total project financial savings
    const totalFinancialSavings =
      qualityCostSavings + financialSavings + fteBenefits;

    // Get values from form for costs - ensure they're numbers
    const oneOffPeopleCost =
      parseFloat(charterForm.getValues("oneOffPeopleCost").toString()) || 0;
    const oneOffTechnologyCost =
      parseFloat(charterForm.getValues("oneOffTechnologyCost").toString()) || 0;
    const oneOffOtherCost =
      parseFloat(charterForm.getValues("oneOffOtherCost").toString()) || 0;
    const capexCost =
      parseFloat(charterForm.getValues("capexCost").toString()) || 0;

    // Calculate total project costs
    const totalProjectCosts =
      oneOffPeopleCost + oneOffTechnologyCost + oneOffOtherCost + capexCost;
    ...
    charterForm.setValue(
      "totalFinancialSavings",
      Math.round(totalFinancialSavings).toString(),
    );
    charterForm.setValue(
      "totalProjectCosts",
      Math.round(totalProjectCosts).toString(),
    );
    charterForm.setValue(
      "projectNetValue",
      Math.round(projectNetValue).toString(),
    );
  ...
  const addRequirement = () => {
    ...
    if (
      lastReq.requirement.trim() !== "" ||
      lastReq.customerRequirement.trim() !== ""
    ) {
      setRequirements([
        ...requirements,
        {
  ...
  const calculateGap = (importance: number, ctqValue: string | number) => {
    ...
    const ctqNumber =
      typeof ctqValue === "string" ? Number(ctqValue) : ctqValue;
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const projectTitle =
        charterForm.watch("projectTitle") || "Project Charter";
      ...
      const projectCategoryValue =
        currentProject?.projectCategory || "Process Improvement";
      ...
      const removeElements = clone.querySelectorAll(
        "#project-image-section, button, .html2canvas-hide:not(.stakeholder-table-container .html2canvas-hide)",
      );
      ...
      if (stakeholderSection && stakeholderSection instanceof HTMLElement) {
        ...
        const labelElement =
          stakeholderSection.querySelector("h3, .section-title");
      ...
      if (teamMemberSection && teamMemberSection instanceof HTMLElement) {
        // Get the label element from the section
        const labelElement =
          teamMemberSection.querySelector("h3, .section-title");
      ...
      if (projectTypeContainer) {
        ...
        const existingShowElements =
          projectTypeContainer.querySelectorAll(".html2canvas-show");
      ...
      if (projectCategoryContainer) {
        // Remove any existing html2canvas-show elements that might be hiding
        const existingShowElements =
          projectCategoryContainer.querySelectorAll(".html2canvas-show");
        existingShowElements.forEach((el) => el.remove());

        // Add it to the container
        projectCategoryContainer.appendChild(
          createValueDiv(projectCategoryValue),
        );
      }
      ...
      if (projectLeaderBeltContainer) {
        ...
        const existingShowElements =
          projectLeaderBeltContainer.querySelectorAll(".html2canvas-show");
        existingShowElements.forEach((el) => el.remove());

        // Add the value to the container
        projectLeaderBeltContainer.appendChild(
          createValueDiv(projectLeaderBeltLevel),
        );
        console.log(
          "Setting project leader belt level value:",
          projectLeaderBeltLevel,
        );
      }
      ...
      if (coachBeltContainer) {
        // Remove any existing html2canvas-show elements
        const existingShowElements =
          coachBeltContainer.querySelectorAll(".html2canvas-show");
        existingShowElements.forEach((el) => el.remove());
      ...
      showElements.forEach((el) => {
        if (el instanceof HTMLElement) {
          ...
          if (dataField === "projectType") {
            ...
            console.log(
              "Setting project leader belt level value in PDF:",
              projectLeaderBeltLevel,
            );
          } else if (dataField === "coachBeltLevel") {
            el.textContent = projectCoachBeltLevel;
            console.log(
              "Setting coach belt level value in PDF:",
              projectCoachBeltLevel,
            );
          }
        }
      });
      ...
      if (costsSection) {
        ...
        const originalCostsSection = charterElement.querySelector(
          "#project-costs-accordion",
        );
        const isExpanded =
          originalCostsSection?.getAttribute("data-state") === "open";

        if (isExpanded) {
          console.log(
            "Project costs section is expanded - applying special handling",
          );

          // Force the content to be visible
          const content = costsSection.querySelector(
      ...
      try {
        ...
        if (imgHeight <= pdfHeight - 2 * margin) {
          ...
          pdf.text("Page 1 of 1", pdfWidth / 2, pdfHeight - 5, {
            align: "center",
          });
        } else {
          ...
          while (currentPage < totalPages) {
            ...
            if (ctx) {
              ...
              ctx.drawImage(
                canvas,
                0,
                position * (canvas.width / imgWidth), // Source x, y
                canvas.width,
                pageCanvasHeight, // Source width, height
                0,
                0, // Destination x, y
                canvas.width,
                pageCanvasHeight, // Destination width, height
              );
  ...
  return (
    <div className="space-y-6">
      ...
      <Card id="project-charter">
        ...
        <CardContent>
          <form
            onSubmit={charterForm.handleSubmit(handleSaveCharter)}
            className="space-y-4"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="html2canvas-hide">
                        <Select
                          onValueChange={(value) =>
                            charterForm.setValue("projectType", value)
                          }
                          value={
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="Black Belt">
                              Black Belt
                            </SelectItem>
                            <SelectItem value="Green Belt">
                              Green Belt
                            </SelectItem>
                            <SelectItem value="Yellow Belt">
                              Yellow Belt
                            </SelectItem>
                            <SelectItem value="White Belt">
                              White Belt
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      ...
                      <div
                        className="html2canvas-show font-normal border rounded-md p-2 mt-1"
                    ...
                    <div>
                      ...
                      <div className="html2canvas-hide">
                        <Select
                          onValueChange={(value) =>
                            charterForm.setValue("projectCategory", value)
                          }
                          value={
                            charterForm.watch("projectCategory") ||
                            "Process Improvement"
                          }
                        >
                          <SelectTrigger className="w-full">
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="Process Redesign">
                              Process Redesign
                            </SelectItem>
                            <SelectItem value="Process Design">
                              Process Design
                            </SelectItem>
                            <SelectItem value="Project Scoping">
                              Project Scoping
                            </SelectItem>
                            <SelectItem value="Kaizen">Kaizen</SelectItem>
                            <SelectItem value="Quick Action">
                              Quick Action
                            </SelectItem>
                          </SelectContent>
                        </Select>
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="html2canvas-hide">
                        <Select
                          onValueChange={(value) =>
                            charterForm.setValue("beltLevel", value)
                          }
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="Master Black Belt">
                              Master Black Belt
                            </SelectItem>
                  ...
                  <div>
                    <Label htmlFor="financialController">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="html2canvas-hide">
                        <Select
                          onValueChange={(value) =>
                            charterForm.setValue("coachBeltLevel", value)
                          }
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        value={
                          ...
                          (currentProject?.targetEndDate
                            ? new Date(currentProject.targetEndDate)
                                .toISOString()
                                .split("T")[0]
                            : "")
                        }
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="savingsPerYear">
                      Quality Cost Savings (p.a.) ({currency})
                    </Label>
                    <Input
                      id="savingsPerYear"
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Annual cost savings expected from quality improvements
                    </p>
                  </div>
                  <div>
                    <Label htmlFor="workingCapitalGains">
                      Working Capital Gains (Cash) ({currency})
                    </Label>
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const wacc =
                          parseFloat(charterForm.getValues("waccPercentage")) /
                            100 || 0;
                        const financialSavings = Math.round(
                          wcg * wacc,
                        ).toString();
                        charterForm.setValue(
                          "financialSavings",
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          const wcg =
                            parseFloat(
                              charterForm.getValues("workingCapitalGains"),
                            ) || 0;
                    ...
                    <div>
                      <Label htmlFor="financialSavings">
                        Financial Savings (p.a.) ({currency})
                      </Label>
                      <Input
                        id="financialSavings"
                        readOnly
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-1">
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                handleFteParamChange("timeUnit", e.target.value)
                              }
                              data-pdf-value={
                                fteParams.timeUnit === "day"
                                  ? "Per Day"
                                  : fteParams.timeUnit === "week"
                                    ? "Per Week"
                                    : "Per Month"
                              }
                            >
                          ...
                          <div>
                            <Label htmlFor="savedHours" className="text-xs">
                              Hours Saved
                            </Label>
                            <Input
                              id="savedHours"
                      ...
                      <div className="pt-2 border-t border-gray-200 mb-3">
                        ...
                        <div className="p-3 mt-2 bg-blue-50 border border-blue-200 rounded-md mb-3">
                          <div>
                            <p className="text-sm font-medium">
                              Calculated FTE Value:{" "}
                              <span className="text-blue-600 font-bold">
                                {fteParams.calculatedFte.toFixed(3)} FTE
                              </span>
                            </p>
                            <p className="text-xs text-gray-600 mt-1">
                              Based on the calculated time saved across the
                              organization
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="html2canvas-show p-3 mt-2 border border-gray-200 rounded-md mb-3">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <p className="text-sm font-medium mb-1">
                                FTE Cost per Year ({currency})
                              </p>
                              <div className="p-2 border border-gray-200 rounded-md bg-white text-sm">
                                {formatCurrency(
                                  parseFloat(
                                    fteParams.fteCostPerYear.toString(),
                                  ),
                                  currency,
                                )}
                              </div>
              ...
              <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-md">
                <div className="flex justify-between items-center">
                  <Label
                    htmlFor="totalFinancialSavings"
                    className="font-medium text-green-800 text-lg"
                  >
                    Total Project Financial Savings (p.a.) ({currency})
                  </Label>
                  <Input
                ...
                <p className="text-sm text-green-600 mt-1">
                  Sum of Quality Cost Savings + Financial Savings + FTE Benefits
                </p>
              </div>
              ...
              <div className="mt-6">
                <div className="flex justify-between items-center mb-2">
                  <Label htmlFor="softBenefits" className="text-base">
                ...
                {softBenefits.length > 0 ? (
                  ...
                  <div className="text-sm text-gray-500 italic mb-3 p-3 border border-dashed rounded-md">
                    No soft benefits added yet. Add benefits that can't be
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Add employee, customer, process, and growth & learning
                  benefits that can't be measured financially. Maximum{" "}
                  {softBenefits.length}/4 benefits.
                </p>

                {/* Dialog for adding new soft benefit */}
                <Dialog
                  open={showSoftBenefitDialog}
                  onOpenChange={setShowSoftBenefitDialog}
                >
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="mb-2 flex items-center gap-2">
                          ...
                          <span className="text-sm">
                            {getCategoryLabel(newBenefitCategory)}
                          </span>
                        </div>
                        <Select
            ...
            <div className="mt-6">
              ...
              <div className="flex justify-between items-center p-3 bg-gray-100 rounded-md transition-colors">
                <h3 className="text-lg font-medium">
                  Project Costs & Financial Metrics
                </h3>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() =>
                    setIsFinancialSectionExpanded(!isFinancialSectionExpanded)
                  }
                >
                    ...
                    <div>
                      ...
                      <div className="mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            <Label htmlFor="oneOffPeopleCost">
                              People ({currency})
                            </Label>
                            <Input
                              id="oneOffPeopleCost"
                          ...
                          <div>
                            <Label htmlFor="oneOffTechnologyCost">
                              Technology ({currency})
                            </Label>
                            <Input
                        ...
                        <div className="mt-2">
                          <Label htmlFor="oneOffOtherExplanation">
                      ...
                      <div className="mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            <Label htmlFor="capexExplanation">
                              Please explain
                            </Label>
                            <Textarea
                              id="capexExplanation"
                      ...
                      <div className="p-3 bg-gray-50 border border-gray-200 rounded-md mb-6">
                        ...
                        <p className="text-xs text-gray-600 mt-1">
                          Sum of all one-off and CAPEX costs
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="p-4 bg-blue-50 border border-blue-200 rounded-md">
                          <div className="flex flex-col">
                            <Label
                              htmlFor="projectNetValue"
                              className="font-medium text-blue-800 mb-2"
                            >
                              Project Net Value ({currency})
                            </Label>
                            <Input
                              id="projectNetValue"
                              readOnly
                            ...
                            <p className="text-xs text-blue-600 mt-1">
                              Total Project Financial Savings (p.a.) - Total
                              Project Costs
                            </p>
                          </div>
                        </div>
          ...
          <form onSubmit={sipocForm.handleSubmit(handleSaveSipoc)}>
            <div className="flex items-center space-x-2 mb-4">
              ...
              <input
                type="text"
                id="processName"
                className="px-3 py-1 border rounded-md text-sm w-[640px]"
                placeholder="Enter process name"
                {...sipocForm.register("processName")}
                defaultValue={sipoc?.sipoc?.processName || ""}
              />
            </div>

            <p className="text-sm text-gray-500 mb-4">
              A SIPOC is a high-level process map (helicopter view). It
              identifies a process's suppliers, inputs, outputs, and customers.
              The process described in a SIPOC is the one within the project
              scope. It is recommended to describe your SIPOC in a minimum of 3
              and a maximum of 7 steps.
            </p>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="overflow-x-auto">
            <table className="w-[99%] mx-auto divide-y divide-gray-200">
              <thead>
                <tr className="w-full">
                  <th className="px-2 py-1 text-center text-xs font-medium uppercase tracking-wider bg-blue-100 text-blue-800 border-r-2 border-white w-[6%]">
                    Customer Requirement
                  </th>
                  <th className="px-2 py-1 text-center text-xs font-medium uppercase tracking-wider bg-emerald-100 text-emerald-800 border-r-2 border-white w-[6%]">
                    Customer Need
                  </th>
                  <th className="px-2 py-1 text-center text-xs font-medium uppercase tracking-wider bg-amber-100 text-amber-800 border-r-2 border-white w-[6%]">
                    Importance (1-5)
                  </th>
                  <th className="px-2 py-1 text-center text-xs font-medium uppercase tracking-wider bg-purple-100 text-purple-800 border-r-2 border-white w-[6%]">
                    Critical to Quality (CTQ)
                  </th>
                  <th className="px-2 py-1 text-center text-xs font-medium uppercase tracking-wider bg-gray-100 text-gray-800 w-[6%]">
                    Actions
                  </th>
                </tr>
              </thead>
                  ...
                  <tr key={index}>
                    ...
                    <td className="px-4 py-2 border-r-2 border-white">
                      <Input
                        type="text"
                        value={
                          req.satisfaction ? req.satisfaction.toString() : ""
                        }
                        onChange={(e) => {
                    ...
                    <td className="px-4 py-2">
                      {index === requirements.length - 1 &&
                      (req.requirement || req.customerRequirement) ? (
                        <Button
                          variant="ghost"
                          size="sm"
          ...
          <div className="mt-4">
            <Button
              ...
              disabled={
                ...
                requirements.every(
                  (r) => !r.requirement && !r.customerRequirement,
                )
              }
            >
              {saveRequirementsMutation.isPending
