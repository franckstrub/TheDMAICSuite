...
export function PControlCard({ projectId, ctqName }: PControlCardProps) {
  ...
  const dataQuery = useQuery({
    queryKey: [`/api/projects/${projectId}/spc/p/${encodeURIComponent(ctqName)}`],
    retry: false,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: { 
      ...
      return apiRequest(
        'POST',
        `/api/projects/${projectId}/spc/p/${encodeURIComponent(ctqName)}`,
        payload
      );
    },
    onSuccess: () => {
      toast({
        title: "Data saved",
        description: "P control card data saved successfully.",
      });
  ...
  const saveAiAnalysisMutation = useMutation({
    mutationFn: async (analysis: string) => {
      return apiRequest(
        'PATCH',
        `/api/projects/${projectId}/spc/p/${encodeURIComponent(ctqName)}/ai-analysis`,
        { aiAnalysis: analysis }
      );
    },
    onSuccess: () => {
      toast({
        ...
        description: "P chart AI analysis saved successfully.",
      });
      queryClient.invalidateQueries({
        queryKey: [`/api/projects/${projectId}/spc/p/${encodeURIComponent(ctqName)}`]
      });
  ...
  const generateAIAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const totalDefective = validData.reduce((a, b) => a + b, 0);
      ...
      const pValues = validData.map((c, i) => c / validSampleSizesArr[i]);
      const avgUCL = pBar + 3 * Math.sqrt(pBar * (1 - pBar) / avgSampleSize);
      const avgLCL = Math.max(0, pBar - 3 * Math.sqrt(pBar * (1 - pBar) / avgSampleSize));
      
      const outOfControl = validData
        .map((_, i) => (pValues[i] > avgUCL || pValues[i] < avgLCL) ? i + 1 : -1)
        .filter(i => i !== -1);
      ...
      const stageStatsForAI = stagesEnabled && localValidIndices.length > 0 ? (() => {
        const stageMap = new Map<string, { defective: number[]; sampleSizes: number[] }>();
        localValidIndices.forEach((originalIdx, validIdx) => {
          const stage = stageValues[originalIdx] || '';
          if (stage === '') return;
          if (!stageMap.has(stage)) stageMap.set(stage, { defective: [], sampleSizes: [] });
          stageMap.get(stage)!.defective.push(validData[validIdx]);
          stageMap.get(stage)!.sampleSizes.push(validSampleSizesArr[validIdx]);
        });
        return Array.from(stageMap.entries()).map(([stageName, data]) => {
          const totalDefective = data.defective.reduce((a, b) => a + b, 0);
          const totalN = data.sampleSizes.reduce((a, b) => a + b, 0);
          const stagePBar = totalDefective / totalN;
          const avgN = totalN / data.defective.length;
          const stagePUCL = stagePBar + 3 * Math.sqrt(stagePBar * (1 - stagePBar) / avgN);
          const stagePLCL = Math.max(0, stagePBar - 3 * Math.sqrt(stagePBar * (1 - stagePBar) / avgN));
          return {
            stageName,
            count: data.defective.length,
            mean: stagePBar,
            pBar: stagePBar,
            avgSampleSize: avgN,
            ucl: stagePUCL,
      ...
      const response = await fetch(`/api/projects/${projectId}/spc/p/${encodeURIComponent(ctqName)}/ai-analysis`, {
        method: 'POST',
      ...
      if (data?.assessment) {
        ...
        toast({
          ...
          description: "P chart analysis has been generated successfully",
        });
      } else {
  ...
  const calculatePStats = useCallback(() => {
    if (!hasValidData) return null;

    const n = validDataValues.length;
    const totalDefective = validDataValues.reduce((a, b) => a + b, 0);
    const totalSampleSize = validSampleSizes.reduce((a, b) => a + b, 0);
    const pBar = totalDefective / totalSampleSize;
    const avgSampleSize = totalSampleSize / n;
    
    const pValues = validDataValues.map((c, i) => c / validSampleSizes[i]);
    const UCL = validSampleSizes.map(ni => pBar + 3 * Math.sqrt(pBar * (1 - pBar) / ni));
    const LCL = validSampleSizes.map(ni => Math.max(0, pBar - 3 * Math.sqrt(pBar * (1 - pBar) / ni)));
    const avgUCL = pBar + 3 * Math.sqrt(pBar * (1 - pBar) / avgSampleSize);
    const avgLCL = Math.max(0, pBar - 3 * Math.sqrt(pBar * (1 - pBar) / avgSampleSize));
  ...
  const stats = calculatePStats();

  const isOutOfControl = (pValue: number, ucl: number, lcl: number): boolean => {
    return pValue > ucl || pValue < lcl;
  };
  ...
  const calculateStageStats = useCallback((): StageStats[] => {
    ...
    const processStage = (stageName: string, startIdx: number, endIdx: number) => {
      const stageDefective = validDataValues.slice(startIdx, endIdx + 1);
      const stageSampleSizes = validSampleSizes.slice(startIdx, endIdx + 1);
      if (stageDefective.length < 2) return null;
      
      const totalDefective = stageDefective.reduce((a, b) => a + b, 0);
      const totalN = stageSampleSizes.reduce((a, b) => a + b, 0);
      const pBar = totalDefective / totalN;
      const avgN = totalN / stageDefective.length;
      
      return {
        stageName,
        startIdx: startIdx + 1,
        endIdx: endIdx + 1,
        defectiveCounts: stageDefective,
  ...
  const getPChartPointArrays = () => {
    ...
    stats.pValues.forEach((pValue, i) => {
      ...
      if (stagesEnabled) {
        ...
        if (stageStats) {
          ucl = stageStats.pBar + 3 * Math.sqrt(stageStats.pBar * (1 - stageStats.pBar) / stageStats.avgSampleSize);
          lcl = Math.max(0, stageStats.pBar - 3 * Math.sqrt(stageStats.pBar * (1 - stageStats.pBar) / stageStats.avgSampleSize));
        } else if (isInSinglePointStage(xVal)) {
          skipOOCCheck = true;
        }
      }
      
      if (!skipOOCCheck && isOutOfControl(pValue, ucl, lcl)) {
        outOfControl.x.push(xVal);
        outOfControl.y.push(pValue);
      } else {
        inControl.x.push(xVal);
        inControl.y.push(pValue);
      }
    });
  ...
  const pChartPoints = getPChartPointArrays();

  const generatePChartStageTraces = () => {
    ...
    stageStatsList.forEach((stageStat, idx) => {
      ...
      const stageUCL = stageStat.pBar + 3 * Math.sqrt(stageStat.pBar * (1 - stageStat.pBar) / stageStat.avgSampleSize);
      const stageLCL = Math.max(0, stageStat.pBar - 3 * Math.sqrt(stageStat.pBar * (1 - stageStat.pBar) / stageStat.avgSampleSize));
      // Convert to percentage
      const stageUCLPct = stageUCL * 100;
      const stageLCLPct = stageLCL * 100;
      const stageCLPct = stageStat.pBar * 100;
      ...
      traces.push({
        ...
        type: 'scatter',
        mode: 'lines',
        name: idx === 0 ? 'Centerline (p̄)' : undefined,
        showlegend: idx === 0,
  ...
  const generatePChartStageLimitLabels = () => {
    if (!stagesEnabled || stageStatsList.length === 0) return [];
    ...
    stageStatsList.forEach(stageStat => {
      ...
      const clPct = stageStat.pBar * 100;
  ...
  if (stats) {
    ...
    const uclPct = stats.avgUCL * 100;
    const lclPct = stats.avgLCL * 100;
    const pValuesPct = stats.pValues.map(v => v * 100);
    const outOfControlYPct = pChartPoints.outOfControl.y.map(v => v * 100);
    
    if (stagesEnabled && stageStatsList.length > 0) {
      pChartTraces.push(...generatePChartStageTraces());
    } else {
      pChartTraces.push({
        x: chartXIndices,
    ...
    if (pChartPoints.outOfControl.x.length > 0) {
      pChartTraces.push({
        x: pChartPoints.outOfControl.x,
        y: outOfControlYPct,
        type: 'scatter',
        mode: 'markers',
        name: 'Out of Control',
        marker: { color: '#dc2626', size: 10, symbol: 'circle' },
        hovertemplate: 'Sample %{x}<br>p: %{y:.2f}% (OOC)<extra></extra>',
      });
    }
  }
  ...
  const pChartLayout = {
    ...
    annotations: [
      ...
      ...(stagesEnabled ? generatePChartStageLimitLabels() : generateGlobalLimitLabels()),
    ],
                ...
                {stagesEnabled && stageStatsList.length > 0 ? (
                  <div className="mb-4 space-y-2">
                    {stageStatsList.map((stageStat) => {
                      const stageUCL = stageStat.pBar + 3 * Math.sqrt(stageStat.pBar / stageStat.avgSampleSize);
                      const stageLCL = Math.max(0, stageStat.pBar - 3 * Math.sqrt(stageStat.pBar / stageStat.avgSampleSize));
                      return (
                        <div key={stageStat.stageName} className="border rounded-lg p-3">
                          ...
                          <div className="grid grid-cols-4 gap-3 text-sm">
                            ...
                            <div className="bg-green-50 p-2 rounded">
                              <Label className="text-xs text-gray-600">CL (p̄)</Label>
                              <div className="font-semibold text-green-700">{(stageStat.pBar * 100).toFixed(2)}%</div>
                            </div>
                            <div className="bg-blue-50 p-2 rounded">
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-4 text-sm">
                    ...
                    <div className="bg-green-50 p-3 rounded">
                      <Label className="text-xs text-gray-600">Centerline (p̄)</Label>
                ...
                <Plot
                  ...
                  config={{
                    ...
                    toImageButtonOptions: {
                      format: 'png',
                      filename: `P_Control_Chart_of_${ctqName}`,
                      height: 500,
                      width: 800,
                      scale: 1
                    }
