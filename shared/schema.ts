...
// Xbar-S Control Card Data for SPC (Statistical Process Control)
export const xbarSControlCardData = pgTable(
  "xbar_s_control_card_data",
  {
    id: serial("id").primaryKey(),
    organizationId: integer("organization_id")
      .references(() => organizations.id)
    ...
    // Subgroup size (used when constantSubgroupSize is true, typically 2-25)
    subgroupSize: integer("subgroup_size").default(5),
...
export const insertXbarSControlCardDataSchema = createInsertSchema(xbarSControlCardData).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
// U Control Card Data (U Chart - for defects per unit with VARIABLE sample sizes)
export const uControlCardData = pgTable(
  "u_control_card_data",
  {
    id: serial("id").primaryKey(),
    organizationId: integer("organization_id")
    ...
    // Sample sizes array (variable n for U chart - array of sample sizes for each observation)
...
export const insertUControlCardDataSchema = createInsertSchema(uControlCardData).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
