...
export const multipleSampleHypothesisConfig = pgTable(
  "multiple_sample_hypothesis_config",
  {
    ...
    // Statistical parameter enablers
    enableMeanTest: boolean("enable_mean_test").default(false),
    enableMeanMultipleSPower: boolean("enable_mean_multiple_s_power").default(false),
    enableVarianceTest: boolean("enable_variance_test").default(false),
    enableMedianTest: boolean("enable_median_test").default(false),

    // Power analysis parameters
    powerPower: text("power_power").default("0.90"),
    powerAlpha: text("power_alpha").default("0.05"),
    powerNbrDistri: integer("power_nbr_distri").default(2),
    powerDifference: real("power_difference").default(0),
    powerStdev: real("power_stdev").default(0),
    ...
    // Dataset storage
    datasets: jsonb("datasets").$type<Array<Array<{ indexNumber: number; dataValue: number }>>>().default([]),
    datasetDescriptions: jsonb("dataset_descriptions").$type<string[]>().default([]),

    lastUpdated: timestamp("last_updated").notNull().defaultNow(),
  },
  (table) => ({
    uniqueProjectCtq: unique("multiple_sample_unique_project_ctq").on(
      table.projectId,
      table.ctqId,
    ),
  }),
...
export const insertMultipleSampleHypothesisConfigSchema = createInsertSchema(
  ...
  testType: true,
  enableMeanTest: true,
  enableMeanMultipleSPower: true,
  enableVarianceTest: true,
  enableMedianTest: true,
  powerPower: true,
  powerAlpha: true,
  powerNbrDistri: true,
  ...
  alternateMean: true,
  alternateVariance: true,
  alternateMedian: true,
  datasets: true,
  datasetDescriptions: true,
});
